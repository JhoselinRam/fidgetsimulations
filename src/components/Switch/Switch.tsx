...
function Switch({ children, className, ...props }: SwitchProps): JSX.Element {
  return (
    <RASwitch
      className={`flex flex-row items-center gap-2 text-sm group data-[hovered]:cursor-pointer ${className}`}
      {...props}
    >
      <div
        className="w-7 h-4 rounded-full bg-zinc-600 pt-[1.5px] pl-0.5
