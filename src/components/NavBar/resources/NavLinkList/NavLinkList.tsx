...
import { MenuToggleContext } from "../../../../hooks/useMenuToggle/useMenuToggle"

function NavLinkList({ children }: NavLinkListProps): JSX.Element {
  const { isCollapsed, isQueryMeet } = useContext(MenuToggleContext)

  return (
    <ul
      className={`flex flex-col absolute top-full left-0 w-screen pr-0 bg-tuatara-950
